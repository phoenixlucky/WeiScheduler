      ...
      <section class="panel list-panel">
        ...
        <div v-else class="task-list">
          <article v-for="task in filteredTasks" :key="task.id" class="task-row" :class="{ expanded: expanded.has(task.id) }">
            ...
            <div class="task-cell"><span class="method-tag"><UiIcon :name="task.runnerType.startsWith('conda') ? 'database' : task.runnerType === 'cmd' ? 'terminal' : 'spark'" size="12" />{{ runnerLabel[task.runnerType] }}</span></div>

...
<p align="center">
  基于 <b>Cron 表达式</b> 的本地任务调度器 —— 支持 <b>Python / Conda / CMD / BAT / 任意可执行程序</b>，
  开箱即用的数据自动化、定时任务管理工具。
</p>
...
| 特性 | 说明 |
|------|------|
| 🧩 **多执行方式** | 直接 Python、Conda 环境名 / 环境路径、CMD / BAT 命令、任意可执行程序 |
...
| 🔁 **失败自动重试** | 可配置失败重试次数和固定间隔，同一任务不会并发执行 |
| 🔐 **任务环境变量** | 为每个任务配置独立的环境变量，不修改系统环境 |
...
| 执行方式 | 直接 Python / Conda 环境名 / Conda 环境路径 / CMD 或 BAT / 可执行程序 |
| Python / 可执行程序命令路径 | Python 模式必填 `python.exe`；可执行程序填写程序路径或 PATH 中的命令名 |
...
| 工作目录 | 可选；Python / Conda 默认使用脚本所在目录，可执行程序使用进程默认目录 |
| 环境变量 | 可选的 `KEY=VALUE` 键值表，仅对当前任务进程及子进程生效 |
| 失败重试次数 / 间隔 | 可选，失败后额外重试 0–5 次，间隔 0–86400 秒；手动终止不会重试 |
| Cron 表达式 | 五段格式，可视化滑块 + 高级编辑双模式 |

可执行程序使用直接进程启动，不经过 Shell。例如：程序填写 `node`，参数填写 `worker.js --mode "daily"`；或程序填写 `powershell.exe`，参数填写 `-File D:\jobs\sync.ps1`。需要管道、重定向或 Shell 内建命令时，请继续使用 CMD / BAT 类型。
